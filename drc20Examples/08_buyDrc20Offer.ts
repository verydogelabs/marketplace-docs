...
export const buyDrc20Offer = async (offerId: string, address: string) => {
  try {
    const sellerPsdtHex = await getPsdtFromOfferId(offerId, 'drc20')
    ...
    const data = await (window as any).dogeLabs.buyDrc20Offer(offerId, signedPsdtHex, address)
